...
export function TimelineHeader({
  ...
  return (
    <div
      className="sticky top-0 z-50 flex bg-background border-b border-grid-line"
      style={{ height: HEADER_HEIGHT }}
    >
      <div
        className="shrink-0 px-2 flex items-center gap-2 border-r border-border sticky left-0 bg-background"
      ...
      <div className="relative bg-background" style={{ width: totalGridWidth }}>
        <TodayLine style={{ left: todayWeekIndex * WEEK_WIDTH }} />
        <div className="grid" style={{ ...weekGridStyle, height: 32 }}>
          {months.map((m, i) => (
            <div
              key={`${m.label}-${i}`}
              className="px-2 py-2 text-xs font-semibold text-foreground border-r border-grid-line last:border-r-0 truncate"
        ...
        >
          {weeks.map((w, i) => {
            ...
            return (
              ...
              >
                {isToday ? (
                  <span className="bg-yellow-300 text-yellow-900 px-1 py-0.5 rounded-sm">
                    {weekDayLabel(w)}
                  </span>
                ) : (
                  weekDayLabel(w)
